...
@app.route('/')
def health(): return "Bot Gold/Silver/ZKB Ready", 200
...
def get_market_status():
    try:
        ...
        etf_silver = yf.Ticker("ZSIL.SW").history(period="2d") # ETF ZKB Silver sur SWX
...
def auto_monitor():
    while True:
        ...
        if d and MY_CHAT_ID != "929066398":
            # Construction du message selon votre format précis
            msg = (
                ...
                f"📈 ETF ZKB (ZSIL) : `{d['etf_price']:.2f} CHF`\n\n"
                f"🇺🇸 **EN DOLLARS (USD)**\n"
